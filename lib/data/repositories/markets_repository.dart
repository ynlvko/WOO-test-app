...
abstract class MarketsRepository {
  ...
  Future<List<Market>> getMarkets(
    Iterable<Specification<Market>> specifications,
  );
}

class DefaultMarketsRepository implements MarketsRepository {
  @override
  ...
  Future<List<Market>> getMarkets(
    ...
    final allMarkets = await getAllMarkets();
    return allMarkets
        .where(
          (market) => specifications.every(
            (spec) => spec.isSatisfiedBy(market),
          ),
        )
        .toList();
  }
...
final List<Market> markets = _markets.map((e) => Market.fromJson(e)).toList();

const List<Map<String, dynamic>> _markets = [
  {
    "base": 'BTC',
    "quote": 'USDT',
    "type": 'SPOT',
